...
void MyDetecter::FindEllipses(const Mat& img, const Mat& imgRed, const Mat& imgBlue, const Mat& imgYellow, vector<TrafficSign>& trafficSigns) {
	...
	findContours(imgRed, contours_temp, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
	...
	findContours(imgYellow, contours_temp, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
	for each (vector<Point> contour in contours_temp) {
		contours.push_back(contour);	
	}
	...
	findContours(imgBlue, contours_temp, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
	for each (vector<Point> contour in contours_temp) {
		contours.push_back(contour);
	}

	/*inRange(img, Scalar(SCARLAR_LOWER_BLACK), Scalar(SCARLAR_UPPER_BLACK), imgBlack);
	threshold(imgBlack, imgBlack, 127, 255, CV_THRESH_BINARY);
	findContours(imgBlack, contours_temp, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);

	for each (vector<Point> contour in contours_temp) {
		contours.push_back(contour);
	}*/

	/*inRange(img, Scalar(SCARLAR_LOWER_WHITE), Scalar(SCARLAR_UPPER_WHITE), imgWhilte);
	threshold(imgWhilte, imgWhilte, 127, 255, CV_THRESH_BINARY);
	findContours(imgWhilte, contours_temp, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
	...
	for each (vector<Point> contour in contours)
	{
		if (contour.size() >= 5) 
		{
			...
			{
				if (temp.size.width / temp.size.height > RATIO) continue;
				if (temp.center.x - temp.size.height / 2 < 0 || temp.center.x + temp.size.height / 2 > img.cols || temp.center.y - temp.size.width / 2 < 0 || temp.center.y + temp.size.width / 2 > img.rows) continue;
			}
			else
			{
				if (temp.size.height / temp.size.width > RATIO) continue;
				if (temp.center.x - temp.size.width / 2 < 0 || temp.center.x + temp.size.width / 2 > img.cols || temp.center.y - temp.size.height / 2 < 0 || temp.center.y + temp.size.height / 2 > img.rows) continue;
			}
...
void MyDetecter::FindSquares(const Mat& img, const Mat& imgRed, const Mat& imgBlue, const Mat& imgYellow, vector<TrafficSign>& trafficSigns)
{
	//int thresh = 100, N = 2;

	// blur will enhance edge detection
	/*Mat timg;
	medianBlur(img, timg, 3);
...
void MyDetecter::FindTriangles(const Mat& img, const Mat& imgRed, const Mat& imgBlue, const Mat& imgYellow, vector<TrafficSign>& trafficSigns)
{
	// Find contours
	vector<vector<Point> >contours, contours_temp;
	// find contours and store them all as a list
	findContours(imgRed, contours_temp, CV_RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
	for each (vector<Point> contour in contours_temp)
	{
		contours.push_back(contour);
	}
	findContours(imgBlue, contours_temp, CV_RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
	for each (vector<Point> contour in contours_temp)
	{
		contours.push_back(contour);
	}
	findContours(imgYellow, contours_temp, CV_RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
	for each (vector<Point> contour in contours_temp)
	{
		contours.push_back(contour);
	}
...
void MyDetecter::DetectTrafficSigns(const Mat& imgSrc, vector<TrafficSign>& trafficSigns)
{
	...
	/*=============SQUARE=============*/
	//FindSquares(img, imgRed, imgBlue, imgYellow, trafficSigns);

	/*===========TRIANGLE=============*/
	//FindTriangles(img, imgRed, imgBlue, imgYellow, trafficSigns);

	/*============CIRCLE===============*/
	FindEllipses(img,imgRed,imgBlue,imgYellow, trafficSigns);
}
